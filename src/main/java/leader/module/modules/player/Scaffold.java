...
public class Scaffold extends Module {
    ...
    private boolean shouldStopSprint() {
        if (this.isTowering()) {
            ...
            return this.stage <= 0 && this.mode.getValue() != 2;
        }
    }
    ...
    private void updateClutch() {
        ...
        if (this.clutchActive) {
            ...
            if (isStuckPhase) {
                Leader.moduleManager.getModule(Stuck.class).setEnabled(true);
            } else {
                Leader.moduleManager.getModule(Stuck.class).setEnabled(false);
            }
        }
    }
    ...
    private boolean isOnEdge() {
        ...
        if (xOff < threshold || xOff > 1.0 - threshold || zOff < threshold || zOff > 1.0 - threshold) {
            ...
            if (checkX != MathHelper.floor_double(mc.thePlayer.posX) || checkZ != MathHelper.floor_double(mc.thePlayer.posZ)) {
                ...
                if (BlockUtil.isReplaceable(adjacentBelow)) {
                    return true;
                }
            }
        }
    ...
    public void onUpdate(UpdateEvent event) {
        if (this.isEnabled() && event.getType() == EventType.PRE) {
                    ...
                    if (!canForward) {
                        ...
                        if (snapLocked) {
                            ...
                            if (!snapForward) {
                                ...
                                snapForwardTimer++;
                                if (snapForwardTimer >= limitTicks.getValue()) {
                                    snapForward = false;
                                    snapLocked = true;
                                    snapForwardTimer = 0;
                                }
            ...
            if (this.canPlace()) {
                ...
                if (this.mode.getValue() == 2) {
                    if (snapForward) {
                        this.yaw = RotationUtil.quantizeAngle(getCurrentYaw());
                        this.pitch = 80.0F;
                        this.canRotate = true;
                    } else {
                        if (!snapRotation.getValue()) {
                            this.yaw = RotationUtil.quantizeAngle(getCurrentYaw() + 180.0F);
                            this.pitch = 85.0F;
                            this.canRotate = true;
                        }
                    }
                ...
                if (this.mode.getValue() == 2 && snapForward) {
                    blockData = null;
                }
                ...
                if (!legitMode && this.rotationMode.getValue() != 0 && this.mode.getValue() != 2) {
                    ...
                    if (speedLimit.getValue() && pendingSpeedLimitRot && !mc.thePlayer.onGround && airTicks >= speedLimitTicks.getValue()) {
                        ...
                        this.yaw = RotationUtil.quantizeAngle(event.getYaw() + yawDelta * RandomUtil.nextFloat(0.98F, 0.99F));
                        this.pitch = RotationUtil.quantizeAngle(RandomUtil.nextFloat(30.0F, 80.0F));
                        this.forwardRotateTicksLeft = forwardRotationTicks.getValue();
                        this.rotationTick = 0;
                        this.towering = true;
                        pendingSpeedLimitRot = false;
                        airTicks = 0;
    ...
    public void onRender(Render2DEvent event) {
        if (this.isEnabled()) {
            ...
            if (bPSRender.getValue()) {
                ...
                int fgColor = currentBps > 5.92F ?
                        new Color(255, 50, 50, 200).getRGB() :
                        new Color(0, 200, 255, 200).getRGB();
